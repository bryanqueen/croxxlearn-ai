...
export const sendVerifyOTP = async (email, otp) => {
    
    // Read the HTML template
    const templatePath = path.join(process.cwd(), 'lib', 'email-verification.html');
    let html = fs.readFileSync(templatePath, 'utf8');
    ...
    //Send mail
    await transporter.sendMail({
        from: 'croxxlearn ai verify@croxxlearn.com',
        to: email,
